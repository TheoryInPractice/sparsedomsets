...
void create_model(CIntDiGraph const& H, std::vector<std::vector<int>> const& groups, std::vector<int> const& landmarks) {
  ...
  for (int k = 0; k < effective_landmarks.size(); ++k) {
    auto u = effective_landmarks[k];
    int nn = candidates[u].size();
    if (nn <= 1) continue;

    // square of the sum (expand all terms)
    std::string padding = "";
    for (int i = 0; i < nn; ++i) {
      auto x = candidates[u][i];
      for (int j = i; j < nn; ++j) {
        auto y = candidates[u][j];
        long long w = (i == j ? 1LL : 2LL) * groups[x].size() * groups[y].size();  // weight

        if (x == u && y == u) {
          // constant (we can ignore)
        } else if (x == u || y == u) {
          // linear terms
          if (w == 1) {
            printf("%sx_%d_%d", padding.c_str(), x + y - u, u);
          } else {
            printf("%s%lld x_%d_%d", padding.c_str(), w, x + y - u, u);
          }
          padding = " + ";
        } else {
          // quadratic terms
          if (w == 1) {
            printf("%s[ x_%d_%d * x_%d_%d ]", padding.c_str(), x, u, y, u);
          } else {
            printf("%s[ %lld x_%d_%d * x_%d_%d ]", padding.c_str(), w, x, u, y, u);
          }
          padding = " + ";
        }
      }
    ...
    if (k < effective_landmarks.size() - 1) printf(" + \n");
  }
  printf("\n");
  ...
  // (1) Each vertex must map to one landmark.
  for (int i = 0; i < n; ++i) {
  ...
  // (2) There must be an induced path from the landmark in each part.
  for (int i = 0; i < n; ++i) {
    if (choices[i].size() < 2) continue;  // contracted, landmarks, or unreachable from any landmark
