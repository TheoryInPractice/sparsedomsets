...
from typing import List, Set, Dict, Tuple, IO, Union
import os
import tempfile
import subprocess
...
def write_to_gxt_relabeled(fp: IO[str], graph: Graph) -> List[int]:
    """
    Relabels all vertices to 0-indexed consecutive integers and
    outputs the graph in GXT format.
    """
    vertex_table: List[int] = []
    vertex_table_rev: Dict[int, int] = {}
    edges = SortedSet()
    ...
    for u, v, _ in graph.arcs():
        for x in [u, v]:
            if x not in vertex_table_rev:
                xx = len(vertex_table)
                vertex_table += [x]
                vertex_table_rev[x] = xx

        uu = vertex_table_rev[u]
        vv = vertex_table_rev[v]
        edges.add((uu, vv))
        edges.add((vv, uu))
    ...
    assert len(vertex_table) <= n, 'graph cannot have more than n vertices'

    # add isolates
    for v in graph:
        if v not in vertex_table_rev:
            vertex_table += [v]

    return vertex_table


def rdomset_sparse(graph: Graph, radius: int, strategy=74, seed=1, num_threads=8) -> SortedSet:
    """
    Finds a sparse dominating set using the `nbrprt` program.

    strategy:
        g12n (degree-plus) -> 1 + 2 * 8 + 64 = 81
        g21n (ratio-plus)  -> 2 + 1 * 8 + 64 = 74
    """
    ret = SortedSet()
    ...
    # create a temporary GXT file
    domset_input = tempfile.NamedTemporaryFile('w', dir=tempdir.name, suffix='.gxt', delete=False)
    vertex_table = write_to_gxt_relabeled(domset_input, graph)
    domset_input.close()

    # run the domset program
    domset_path = os.path.join(os.path.expanduser('~'), 'proj', 'TiP-internal', 'metagenomics-fall-2020', 'bin', 'domset')
    domset_output = tempfile.NamedTemporaryFile(dir=tempdir.name, delete=False)
    domset_output.close()
    subprocess.run([domset_path, domset_input.name, str(radius), str(strategy), str(seed), str(num_threads), domset_output.name])

    # collect result
    with open(domset_output.name) as domset_result:
        for line in domset_result:
            line = line.strip()
            if line:
                ret.add(vertex_table[int(line)])
...
def domination_graph_balanced(graph: Graph, domset: Set[int], radius: int, use_greedy: bool = False, strategy: int = 1, seed: int = 0) -> Tuple[DictGraph, SortedDict]:
    """
    Finds a balanced neighborhood partitioning using Gurobi.
    """

    assigned_dominator: Dict[int, int] = {v: -1 for v in graph}

    # work in a temporary directory
    tempdir = tempfile.TemporaryDirectory(dir='.')

    # create a temporary GXT file
    graph_input = tempfile.NamedTemporaryFile('w', dir=tempdir.name, suffix='.gxt', delete=False)
    vertex_table = write_to_gxt_relabeled(graph_input, graph)
    vertex_table_rev = {v: i for i, v in enumerate(vertex_table)}
    graph_input.close()

    # create a domset file
    domset_input = tempfile.NamedTemporaryFile('w', dir=tempdir.name, suffix='.txt', delete=False)
    for u in domset:
        domset_input.write(f'{vertex_table_rev[u]}\n')
    domset_input.close()

    result_file = tempfile.NamedTemporaryFile('w', dir=tempdir.name, suffix='.txt', delete=False)

    if use_greedy:
        nbrprt_path = os.path.join(os.path.expanduser('~'), 'proj', 'TiP-internal', 'metagenomics-fall-2020', 'bin', 'nbrprt')
        subprocess.run([nbrprt_path, graph_input.name, domset_input.name, str(strategy), str(seed), result_file.name], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    else:
        # create a Gurobi model file
        make_qp_path = os.path.join(os.path.expanduser('~'), 'proj', 'TiP-internal', 'metagenomics-fall-2020', 'bin', 'make_qp')
        model_file = tempfile.NamedTemporaryFile('w', dir=tempdir.name, suffix='.lp', delete=False)
        subprocess.run([make_qp_path, graph_input.name, domset_input.name], stdout=model_file)
        model_file.close()

        # run Gurobi
        solution_file = tempfile.NamedTemporaryFile(dir=tempdir.name, suffix='.sol', delete=False)
        solution_file.close()
        log_file = tempfile.NamedTemporaryFile(dir=tempdir.name, suffix='.log', delete=False)
        log_file.close()

        gurobi_cl = 'gurobi_cl'
        gurobi_time_limit = 3600  # 1 hour
        subprocess.run(
            [gurobi_cl, f'TimeLimit={gurobi_time_limit}', f'ResultFile={solution_file.name}', f'LogFile={log_file.name}', model_file.name],
            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )

        # parse solution
        parse_qp_path = os.path.join(os.path.expanduser('~'), 'proj', 'TiP-internal', 'metagenomics-fall-2020', 'bin', 'parse_qp')
        subprocess.run(
            [parse_qp_path, graph_input.name, domset_input.name, solution_file.name],
            stdout=result_file, stderr=subprocess.DEVNULL
        )

    # load result
    with open(result_file.name) as part_result:
        for line in part_result:
            tokens = line.strip().split()
            if len(tokens) >= 2:
                u = vertex_table[int(tokens[0])]
                for t in tokens[1:]:
                    v = vertex_table[int(t)]
                    assigned_dominator[v] = u
